...
<summary>💡 <b>v1.6.5 版本更新（2026-07-14）</b></summary>  
   
### 更新内容
   
- **新增消息确认反馈功能（message_ack）**：消息成功转发到群组后，可自动向用户发送确认反馈。支持两种模式：
  - **回复消息模式**：发送一条自定义文本确认消息，管理员回复用户时自动编辑该消息为已读文本。
  - **表情反应模式**：在用户原始消息上添加自定义 emoji 反应，管理员对转发到群组的消息添加任意表情时自动替换为已读表情。
  - 两种模式二选一，可在设置面板中随时切换。
  - 收到消息文本/表情和已读文本/表情均可自定义。
  - 已读检测仅限 `ADMIN_IDS` 管理员触发：管理员在话题内回复消息，或管理员对转发消息添加表情反应。
  - ⚠️ **重要**：表情反应模式需要 webhook 注册时包含 `allowed_updates=["message","edited_message","callback_query","message_reaction"]`，详见下方部署步骤。
...
| **💬 消息确认反馈**     | 消息成功转发后可自动回复或添加表情反应，支持已读状态更新，回复/反应二选一，文本和表情均可自定义                 |
| **⚙️ 前台设置面板**     | 群组内可使用 **/settings** 打开设置面板，便于通过前台快速控制机器人开关、验证、拦截、消息确认反馈等功能         |
...
6.  点击 **部署**，等待部署完成
7.  点击 Cloudflare 当前 Worker 页面右上角的 **编辑代码**
8.  删除所有代码，复制并粘贴本 Github 项目仓库中 **worker.js** 文件的所有代码
9.  点击右上角 **部署**，等待页面底部提示 **版本已保存**，即为部署成功
...
`https://api.telegram.org/bot<你的BOT_TOKEN>/setWebhook?url=https://<你的Worker域名>&allowed_updates=["message","edited_message","callback_query","message_reaction"]`

例如：`https://api.telegram.org/bot12345678:A43eUzq3/setWebhook?url=https://nextgenforward.xxxxxx.workers.dev&allowed_updates=%5B%22message%22%2C%22edited_message%22%2C%22callback_query%22%2C%22message_reaction%22%5D`

如果页面返回 `{"ok":true, "result":true, "description":"Webhook was set"}`，即表示激活成功！

**已设置`WEBHOOK_SECRET`变量：**

`https://api.telegram.org/bot<你的BOT_TOKEN>/setWebhook?url=https://<你的worker域名>/&secret_token=<你的WEBHOOK_SECRET>&allowed_updates=["message","edited_message","callback_query","message_reaction"]`

例如：`https://api.telegram.org/bot12345678:A43eUzq3/setWebhook?url=https://nextgenforward.xxxxxx.workers.dev/&secret_token=Abc_1234-5d6F7G&allowed_updates=%5B%22message%22%2C%22edited_message%22%2C%22callback_query%22%2C%22message_reaction%22%5D`
